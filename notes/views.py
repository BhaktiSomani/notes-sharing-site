...
def admin_home(request):
    ...
    pn = Notes.objects.filter(status = "pending").count() #pn is queryset
...
def upload_notes(request):
    ...
    if request.method == "POST":
        ...
        try:
            Notes.objects.create(user = u,uploadingdate = date.today(),branch = b,subject = s,notesfile= n ,filetype = f,discription=d,status='pending')
            error ="no"
